...
from mf.crud.forms import PermisologyForm
from mf.crud.models import Permisology
from mf.crud.functions import *

class PermisologyListView(LoginRequiredMixin, ValidatePermissionMixin, TemplateView):
    template_name = 'calendar/calendar.html'
    permission_required = 'view_permisology'
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'get_events':
                data = []
                for i in Permisology.objects.using('default').all():
                    valores = {
                        'id': i.id,
                        'title': i.name,
                ...
                perms = ['delete_permisology',]
                group = request.user.groups.first()
                authorized = ValidatePermissions(perms, group)
                if(authorized == False):
                    data['error'] = 'Disculpe, usted no tiene permisos para ejecutar esta acción'
                elif(authorized == True):
                    p = Permisology.objects.using(
