...
from mf.crud.models import Sale
from mf.reports.forms import ReportForm

from django.db.models.functions import Coalesce
from django.db.models import Sum

class ReporteSaleView(TemplateView):
        ...
        try:
            ...
            if action == 'search_report':
                ...
                if len(start_date) and len(end_date):
                    search = search.filter(datejoined__range=[start_date, end_date])
                for s in search:
                    data.append([
                        s.id,
                        s.cli.names,
                        s.datejoined.strftime('%Y-%m-%d'),
