...
$(function () {
    ...
    $('.btnAddType').on('click', function () {
        ...
        modal_title.find('span').html('Nuevo tipo de servicio');
    ...
    $('#data tbody').on('click', 'a[rel="edit"]', function () {
        modal_title.find('span').html('Editar tipo de servicio');
        ...
        let tr = tbTypeServices.cell($(this).closest('td, li')).index();
        let data = tbTypeServices.row(tr.row).data();
        ...
        .on('click', 'a[rel="delete"]', function () {
            ...
            submit_with_ajax_msj(window.location.pathname, 'Notificación', '¿Estas seguro de realizar eliminar el siguiente registro?', parameters, function () {
                alertSweetSuccess('Tipos de Servicios Actualizados');
                setTimeout(tbTypeServices.ajax.reload(), 5000);
            });
        });
