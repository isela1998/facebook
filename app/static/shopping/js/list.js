...
$(function () {
    ...
    $('#i_card_title').removeClass().addClass('text-dark fas fa-credit-card')
    ...
    $('.btnAddShopping').on('click', function () {
        $('input[name="action"]').val('add');
        modal_title.find('span').html('Nueva Compra');
    ...
    $('#data tbody').on('click', 'a[rel="edit"]', function () {
        modal_title.find('span').html('Editar Compra');
        ...
        let tr = tbShopping.cell($(this).closest('td, li')).index();
        let data = tbShopping.row(tr.row).data();
        ...
        .on('click', 'a[rel="delete"]', function () {
            ...
            submit_with_ajax_msj(window.location.pathname, 'Notificación', '¿Estas seguro de eliminar este registro?', parameters, function () {
                alertSweetSuccess('Listado de Compras actualizado');
                setTimeout(tbShopping.ajax.reload(), 5000);
            });
        });
